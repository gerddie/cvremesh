...
int main(int argc, char **argv)
{
	...
	GOptionEntry entries[] = {
		...
		{ "cvopt", 'c', 0, G_OPTION_ARG_NONE, &cvopt, "use centroid voronoy Approximation", NULL}, 
	...
	GtsEnergyClass *klass; 
	if (cvopt) {
		g_message("Use approximate centroid energy");  
		klass = GTS_ENERGY_CLASS(gts_cvenergy_class()); 
	}else{
		g_message("Use Perimeter/Area energy"); 
		klass = GTS_ENERGY_CLASS(gts_paenergy_class()); 
	}
	GtsEnergy *energy = gts_energy_new(klass); 

	
	gts_surface_cluster(in_mesh, nclusters, energy, TRUE, NULL, NULL);
